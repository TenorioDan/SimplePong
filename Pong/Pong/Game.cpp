...
void GE161::Game::setWindowColor(int r, int g, int b)
{
	theGame->window()->red = r;
	theGame->window()->green = g;
	theGame->window()->blue = b;
}
...
void GE161::Game::SendEvent(int eventType){
	if (eventType == 1){
		Event* e = new Event();
		e->type = Event::HIT_EVENT;
		theGame->eventQueue_->eventQueue_.push(e);
	}
}
